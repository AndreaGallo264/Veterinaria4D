...
export default function Payments({price , carProduct , isAdmin}) {
    ...
    const onSubmitAddPayment = e => {
        ...
        if (numbercard.trim() === '' || dateexp.trim() === '' || codesec.trim() === '') {
         ...
         // Agregar Producto
         payment.users = isAdmin.usuario._id
         AddPayment(payment);
    ...
    const AddCardProduct = async (carProduct) => {
        carProduct.forEach(product => {
            ...
            product.users = isAdmin.usuario._id
            /*const request =*/  fetch(process.env.REACT_APP_BACKEND_URL+"addCardProduct", {
                method: 'POST',
